...
@Component
public class CassandraDataInitializer implements CommandLineRunner {

    private final MovieRepository movieRepository;
    private final ActorRepository actorRepository;

    public CassandraDataInitializer(MovieRepository movieRepository, ActorRepository actorRepository) {
    ...
    public void run(String... args) {
        ...
        // Generate 500 unique actors with rich data (denormalized strings for Cassandra)
        ...
        for (int i = 0; i < 500; i++) {
            ...
            for (int j = 0; j < filmCount; j++) {
                if (j > 0) filmography.append(",");
                filmography.append(moviePool[random.nextInt(moviePool.length)]);
            }

            int awardCount = 2 + random.nextInt(4);
            StringBuilder awards = new StringBuilder();
            for (int j = 0; j < awardCount; j++) {
                if (j > 0) awards.append(",");
                awards.append(awardTemplates[random.nextInt(awardTemplates.length)]);
            }
        ...
        // Generate 50000 movies with 8-12 actor IDs each
        java.util.List<String> actorIds = new java.util.ArrayList<>(actorMap.keySet());
        for (int i = 0; i < 50_000; i++) {
            ...
            StringBuilder actorIdsStr = new StringBuilder();
            for (int j = 0; j < actorCount; j++) {
                if (j > 0) actorIdsStr.append(",");
                actorIdsStr.append(actorIds.get(random.nextInt(actorIds.size())));
            }
